.. SpiderPi documentation master file, created by
   sphinx-quickstart on Thu Jul 25 14:39:47 2024.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

===========
AiNova
===========

.. toctree::
   :maxdepth: 2
   :caption: AiNova

   docs/1.Read_first.md
   docs/2.programming_software_installation.md
   docs/3.Factory_Program_Download_Instructions.md
   docs/4.app_remote_control.md
   docs/5.AiNova_Basic_Feature_Course.md
   docs/6.AI_Vision_lesson.md
   docs/7.Python_Extension_Course.md
   docs/8.Arduino_Extension_course.md
   docs/Appendix.md
   Download <https://drive.google.com/drive/folders/10XXr8CeLymUqNXfqsT3URFiQA-t72rDp?usp=sharing>